...
// on click sub category in category drawer
    ...
    const SCANNER_TIMEOUT = 100; // milliseconds between keypresses to consider it a scanner input

    $(document).on('keypress', '[data-barcode-scanner]', function(e) {
        ...
        if (e.which === 13) { // Enter key
            ...
            if (barcodeInput.length > 0) {
                $(this).val(barcodeInput);
                $(this).trigger('change');
                barcodeInput = '';
            }
        } else {
            barcodeInput += String.fromCharCode(e.which);
        }
    });
});
